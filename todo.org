...
*** Prompt 0: Nix Environment Setup
- [ ] Initialize a `flake.nix` file to define the development environment.
- [ ] Ensure the flake provides:
    - [ ] Go (compatible version for the project).
    - [ ] JDK (Java Development Kit).
    - [ ] Android SDK / Platform Tools (or instructions/scripts to manage them if Nix packaging is too volatile).
    - [ ] `gomobile` tool.
- [ ] Set up `pre-commit` hooks configuration (`.pre-commit-config.yaml`).
    - [ ] Include hooks for: `gofmt`/`goimports` (Go), `ktlint` (Kotlin), and standard trailing whitespace/end-of-file checks.
- [ ] Ensure `nix develop` drops into a shell with all tools available and pre-commit hooks installed.
